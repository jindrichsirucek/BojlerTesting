...
// Sync
bool readTemperaturesSyncVersion()
{
  ...
  while(cnt--)
  {
    ...
    for(uint8_t sensorIndex = 0;sensorIndex < MAX_TEMP_SENSORS_COUNT;sensorIndex++)
    {
      if(goodTempSampleCount[sensorIndex] == CORRECT_TEMP_SAMPLES_COUNT || GLOBAL.TEMP.sensors[sensorIndex].sensorConnected == false)
      {
        sensorMeasurmentDoneCount++;
        if(sensorMeasurmentDoneCount == MAX_TEMP_SENSORS_COUNT)
        {
      ...
      if(GLOBAL.TEMP.sensors[sensorIndex].sensorConnected)
      {
        ...
        if(TEMPERATURE_DEBUG) DEBUG_OUTPUT.print((String)(MAX_TEMPERATURE_SAMPLES_COUNT - cnt) + E(". ")+getSensorAddressHexString(GLOBAL.TEMP.sensors[sensorIndex].address)+E(" ")+GLOBAL.TEMP.sensors[sensorIndex].sensorName+E(" temp trial: ") + currentTempSampleValue);
